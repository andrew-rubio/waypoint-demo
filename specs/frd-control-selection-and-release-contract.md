...
- **FR-010-1** A **proposition declaration** captures machine-readable characteristics (LLM use, personal-data handling, autonomy, consequential actions, integrations, human-in-loop, retrieval, deployment surface) with stable IDs. Intake is **deterministic and human-authored**: a schema-driven template is generated with every governance classification `UNRESOLVED`, an accountable human supplies + confirms the mandatory values, and deterministic validation blocks promotion until they are resolved. **No model infers, classifies, or populates a governance value from prose.**
