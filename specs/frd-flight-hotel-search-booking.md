...
- **FR-005-2** The agent calls the RouteStack MCP to search **flights** and **hotels** and presents up to **3 options each**, with a "best" indicator where available. **[UI-REVISED 2026-08-17]** The flight/hotel options card shows a footnote **"Source: RouteStack"**.
- **FR-005-3** Flight options show airline, **flight number**, route, **departure/arrival times**, duration, stops, and whether the price is per traveller. Flights on one of the traveller's saved preferred airlines are ranked first and labelled **"Your Preferred"** (FRD-006). Hotel options show name, **star rating (e.g. "5-star")**, **address**, nightly rate, and whether taxes/fees are included. `[UI-REVISED 2026-08-15]` Selecting a flight (or hotel) adds only that leg to the composer as `Book the first flight (Airline FlightNo)`, then selecting the other appends ` and the first hotel (Name)`; the chosen card shows a selected state (button reads "Selected", highlighted border).
...
- **FR-005-5** Selecting an option triggers **`booking-simulator`**, which returns a deterministic mock confirmation (reference code, itinerary echo) **without** any payment or real reservation. **[GOVERNANCE ADR-013]** Because a booking is a consequential action, the runtime authority gate returns `require_approval`: the agent **pauses** and emits an `approval_request`, and `booking-simulator` runs only after the traveller approves via the approval card (`POST /api/chat/approve`). A denial blocks the booking and no confirmation is produced.
