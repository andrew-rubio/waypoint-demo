...
## Workflow
1. `npm run gov:draft` — draft `proposition.draft.yaml` from the requirement + PRD. An LLM/Copilot may pre-fill it, but high-impact classifications are left **UNRESOLVED** and never invented.
2. Human reviews + resolves the draft (fills high-impact fields, clears `unresolved`), then `npm run gov:promote-proposition` — refuses while any field is unresolved; promotes to `proposition.yaml` only after confirmation.
3. `npm run gov:select` — deterministic control selection from `proposition.yaml` + `controls.yaml`.
4. `npm run gov:contract` — versioned, hash-bound control contract (deterministic hash).
5. `npx tsx src/governance/src/cli.ts approve --approver "<id>"` — human hash-bound approval.
6. `npm run gov:evidence -- --mode ci-authoritative --digest <d>` — collect source-bound evidence.
7. `npm run gov:verify` / `npm run gov:certify` — blocking release decision / deployability.
8. `npm run gov:dossier` · `gov:trace` · `gov:status` · `gov:change-impact` · `gov:learn` — dossier, traceability, lifecycle, change-impact, learning.
9. `npm run gov:demo-failure` / `gov:demo-pass` · `npm run eval:demo-fail` / `eval:demo-pass` — undeclared-MCP + failing-evaluation negative/positive demonstrations.
