...
### Waypoint stack conventions (resolved Phase 1d — see specs/tech-stack.md)
...
- **Tests:** Vitest (unit), Cucumber.js (BDD), Playwright (e2e) — all against the Aspire environment.
- **Money:** GBP default; EUR on request via Currency MCP (store rate + timestamp). Booking is **simulated only**.
