/**
 * Travel-guide knowledge base for the waypoint-data MCP (ADR-008/009). In
 * production `searchByMonth` runs a hybrid vector query over the Azure AI Search
 * `travel-guide` index built from the DK Eyewitness guide "Where To Go When"
 * (src/assets). This deterministic month→destinations dataset — curated from that
 * same guide — is the offline fallback when AI Search is not configured, mirroring
 * the Cosmos/offline profile pattern. Kept in sync with the API-side copy.
 */
...
/** The offline guide picks for a month (fallback when AI Search is not configured). */
export function offlineGuideByMonth(month: string): GuidePassage[] {
  const canonical = MONTH_LOOKUP.get(month.trim().toLowerCase());
  if (!canonical) return [];
  return GUIDE[canonical].map((entry) => ({ ...entry, month: canonical }));
}

/** The full curated dataset (used to seed the AI Search index during ingestion). */
export function allGuidePassages(): GuidePassage[] {
  return MONTHS.flatMap((m) => GUIDE[m].map((entry) => ({ ...entry, month: m })));
}
