...
// ── OPS-VER-001: model/agent version is captured in the GenAI telemetry spans ──
export function versionCaptureAdapter(ctx: EvidenceContext): AdapterResult {
  return sourceScan(
    ctx,
    'src/api/src/telemetry/agent-spans.ts',
    /gen_ai\.request\.model|gen_ai\.response\.model|gen_ai\.system|'model'/,
    'governance:version-adapter',
    'GenAI telemetry captures the model/agent identifiers per turn.',
    'Telemetry does not capture required model/agent version identifiers.',
  );
}
